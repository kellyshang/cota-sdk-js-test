...
const { Collector, Aggregator, generateClaimCotaTx } = require("@nervina-labs/cota-sdk")
...
describe('Claim test', () => {
    ...
    const claim = async (cotaID, tokenIndex) => {
    ...
    const fakeRun = async (cotaID, tokenIndex) => {
        console.log("=====fake run without sending claim=====")
        await claim(cotaID, tokenIndex)
    }

    const realRun = async (cotaID, tokenIndex) => {
        console.log("=====real run sending claim=====")
        let signedTx = await claim(cotaID, tokenIndex)
    ...
    it('case1: expect sending claim successfully after fake sending', async () => {
        console.log(`tokenindex in case1: ${cotaID}, ${tokenIndex}`)
        let txHash = await fakeRealRun(cotaID, tokenIndex)
        console.log("sending claim txHash: ", txHash)
    ...
    it('case2: expect error when claim with owned non-withdrawl cota nft', async () => {
        ...
        readLog("claim.log", "The cota_id and token_index has not withdrawn");
    })

    // TODO: should report with `has not withdrawn` error after fixed
    it('case3: expect error when claim with non-owned non-withdrawl cota nft', async () => {
    ...
    it('case4: expect error when claim with non-existing cotaID nft', async () => {
